 ...
 */
function NPMRegistrySource(baseDir, dependencyName, versionSpec) {
        ...
        function(callback, client) {
            ...
            client.get(url, {auth}, function(err, data, raw, res) {
                if(err) {
                    ...
                    callback(null, Object.assign({}, data, {auth}));
                }
            });
        },
...
NPMRegistrySource.prototype.toNixAST = function() {
    ...
    if (this.auth && this.auth.token) {
      paramExpr.curlOpts = "--header \"Authorization: Bearer " + this.auth.token + "\"";
    }
